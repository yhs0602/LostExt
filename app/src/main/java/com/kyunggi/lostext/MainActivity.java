...
public class MainActivity extends Activity implements Button.OnClickListener
{
					...
					for(;i<total;++i)
					{
						...
						runOnUiThread(new Runnable(){
							...
							try
							{
								...
								String extension = getExtension(files[i]);//=reversedHashMap.get(mimeType);
							...
							//	}
								if(extension!=null&&extension.compareToIgnoreCase("")!=0)
								{
									log+=files[i].getName()+"ext"+extension/*+(mimeType==null?"":mimeType)*/+"\n";
									
									String finalname=GetFileNameWithoutExt(files[i].getAbsolutePath());
									files[i].renameTo(new File(finalname+"."+extension));
								}else{
	...
	public String getExtension(File file) throws RuntimeException
	{
		for(FileExtensionMap fem:extmap)
		{
			if(fem.matches(file))
			{
				return fem.filename;
	...
	{
		// TODO: Implement this method
		int slashIndex=hint.lastIndexOf("/");
